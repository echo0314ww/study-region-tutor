...
设置面板顶部提供“整体功能向导”、“本版本新增向导”和“历史版本向导回顾”入口。应用版本变化后首次打开会先显示整体功能向导，再显示当前版本新增向导；用户可以跳过，也可以随时回到设置页重新查看。历史版本向导回顾会按版本保留以往新增功能说明，适合直接安装最新版的用户补看旧版本变化。
...
设置面板会把非敏感体验设置保存在本机，例如连接模式、服务商选择、手动模型名、代理地址、输入方式、OCR 语言和思考程度。API Key 和代理访问 Token 不会写入渲染层 `localStorage`；代理 Token 仍由主进程的安全存储路径单独处理。
...
开发运行时，项目会读取项目根目录和用户配置目录中的 `.env` / `.env.local`；打包后的应用只读取用户配置目录。优先级统一为：命令行/系统环境变量最高，`.env.local` 次之，`.env` 最后。项目根目录的 `.env.local` 已在 `.gitignore` 中，不会提交到仓库。
...
服务会监听 `0.0.0.0:8787`，并自动监听 `.env.local` / `.env` 的创建、删除、重命名和内容变化。你只要修改 `.env.local` 里的服务商、Key、默认服务商或由 ngrok 脚本写入的 `TUTOR_PUBLIC_PROXY_URL`，代理服务会自动重新加载；用户端刷新设置里的“代理服务商”即可拿到最新配置。若新配置无效，例如删除了 `AI_PROVIDERS` 且没有 `AI_BASE_URL` / `AI_API_KEY` 兜底，代理服务会立即进入配置错误状态，`/health` 和 API 代理接口会返回配置错误，不再继续沿用旧 provider。API Key 不会通过 `/providers` 或 `/models` 返回给用户端。

如果运行中修改 `TUTOR_PROXY_PORT`，代理服务会尝试关闭旧监听并切换到新端口；切换失败时会回到旧端口并把 `/health` 标记为配置错误，避免静默转发到错误端口。短时间内正在进行的流式请求可能会被端口切换中断。
...
这个脚本会读取 `.env.local` 里的 `NGROK_AUTHTOKEN` 和 `TUTOR_PROXY_PORT`，先确认对应端口的本地 `/health` 可访问，再自动配置 ngrok、启动 `ngrok http <端口>`，并把检测到的 HTTPS 公网地址写回 `.env.local` 的 `TUTOR_PUBLIC_PROXY_URL`。写入时会保留 `.env.local` 原有内容，只更新这一行；`npm run proxy:dev` 检测到文件变化后会自动重新加载，并在 `/health` 里显示新的公网地址。
...
如果之后修改了 `.env.local` 里的 `NGROK_AUTHTOKEN` 或 `TUTOR_PROXY_PORT`，`npm run ngrok:dev` 会重新检查本地代理端口，确认可访问后再重启 ngrok 隧道并重新写入 `TUTOR_PUBLIC_PROXY_URL`。ngrok 免费隧道地址可能每次启动都会变化，请以 `.env.local` 中最新的 `TUTOR_PUBLIC_PROXY_URL` 或 `/health` 返回的 `serviceUrls.public` 为准。
...
识别或追问过程中可以点击工具栏里的 `停止`。应用会取消当前请求，第三方 API 请求会通过 AbortSignal 中断；OCR 会在取消后尽快终止当前 worker 或在当前候选结束后丢弃结果。OCR worker 会按语言复用并在空闲后释放，连续识别时通常会比第一次更快。
...
本地 OCR 使用 `tesseract.js`。首次识别中文时可能需要下载 `chi_sim` 语言数据并初始化 worker，因此第一次会慢一些，也需要网络可访问语言数据源。应用会按语言缓存 worker，并在空闲一段时间后自动释放，减少连续截图时的重复初始化成本。
...
坐标采用 Electron 的 DIP 坐标在渲染进程和主进程间传递；主进程会根据每个显示器的 `scaleFactor` 换算为物理像素，再裁剪 `desktopCapturer` 返回的屏幕缩略图。单显示器框选会走单段快速裁剪；跨显示器框选会按显示器交集拆成多个裁剪段，再按用户框选区域合成为一张 PNG。
