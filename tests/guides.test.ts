...
describe('guides', () => {
  it('keeps the product guide focused on the full workflow', () => {
    const guide = productGuide('1.1.2');
  ...
  it('provides release guide content for the current feature update', () => {
    const guide = releaseGuide('1.1.2');

    expect(hasGuideContent('release', '1.1.2')).toBe(true);
    expect(guideDefinition('release', '1.1.2').steps).toHaveLength(4);
  ...
  it('keeps historical release guides for users who install the latest version directly', () => {
    const history = historyGuide('1.1.2');

    expect(hasGuideContent('history', '1.1.2')).toBe(true);
    ...
    expect(history.historyVersions?.map((section) => section.version)).not.toContain('1.1.2');
  });
  ...
  it('orders historical release guides from newest to oldest and hides future entries', () => {
    ...
    expect(releaseGuideVersionList()[0]).toBe('1.1.2');
