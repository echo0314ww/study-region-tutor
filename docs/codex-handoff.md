...
- 构建命令：工作流执行 `npm ci`、`npm run docs:check`、`npm run typecheck`、`npm run lint`、`npm run test`、`npm run security:check`、脚本语法检查和 `npm run publish:win`。脚本语法检查覆盖 `server/proxy-server.mjs`、`server/ngrok-dev.mjs`、`server/runtime-env.mjs` 和 `scripts/sync-release-notes.mjs`。
